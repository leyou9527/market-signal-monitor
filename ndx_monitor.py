...
class Settings:
    ...
    ndx_lookback_days: int
    ndx_60d_threshold_pct: float
    ndx_ytd_threshold_pct: float
...
def load_settings(base_dir: Path) -> Settings:
    ...
    # 这里继续沿用 NDX_* 变量名，是为了兼容已经在线上运行的旧 .env。
    ...
    return Settings(
        ndx_lookback_days=env_int("NDX_LOOKBACK_DAYS", "LOOKBACK_DAYS", default=60),
        ndx_60d_threshold_pct=env_float("NDX_60D_DROP_THRESHOLD_PCT", "DROP_THRESHOLD_PCT", default=7.0),
        ndx_ytd_threshold_pct=env_float("NDX_YTD_DROP_THRESHOLD_PCT", default=7.0),
        ...
        log_file=Path(env("LOG_FILE", default=str(base_dir / "logs" / "ndx-monitor.log"))),
...
def setup_logging(path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("ndx_monitor")
...
def fetch_from_twelvedata(session, symbol: str, settings: Settings) -> list[Bar]:
    ...
    try:
        ...
        index_metric, index_window = build_metric(INDEX_SYMBOL, index_result.bars, settings.ndx_lookback_days, True)
        stock_metrics = [
        ...
        index_60d_triggered = index_metric.drawdown_60d_pct >= settings.ndx_60d_threshold_pct
        index_ytd_triggered = index_metric.ytd_change_pct is not None and index_metric.ytd_change_pct <= -settings.ndx_ytd_threshold_pct
    ...
    plain_lines.extend(
        [
            ...
            f"- 60日回撤: {index_metric.drawdown_60d_pct:.2f}% (阈值 {settings.ndx_60d_threshold_pct:.2f}%)",
            f"- YTD涨跌幅: {index_metric.ytd_change_pct:.2f}% (跌幅提醒阈值 {settings.ndx_ytd_threshold_pct:.2f}%)",
    ...
    chart_block_html = (
        "<img src='cid:ndx_chart' alt='index chart' style='display:block;width:100%;height:auto;border-radius:12px;'>"
          ...
          <table role='presentation' width='100%' cellpadding='0' cellspacing='0' style='max-width:680px;background:#fff;border:1px solid #dbe4ee;border-radius:20px;'>
            ...
            <tr>
              <td style='padding:12px 18px 0;'>
                <div style='font-size:18px;font-weight:800;margin-bottom:10px;'>{INDEX_DISPLAY_NAME}最近 {settings.ndx_lookback_days} 个交易日K线</div>
